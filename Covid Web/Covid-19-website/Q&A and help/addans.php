<html>
    ...
    <!-- header -->
        ...
        $sql='SELECT question FROM question WHERE no='.$q;
        ...
        $row=$res->fetch_assoc();
        echo $row["question"];
        echo '<input name="qno" value='.$q.' hidden>';
        mysqli_close($conn);
        ?>
            <script>
                var txt;
                function f()
                {
                    let x=document.f1.ans.value;
                    if(txt!=x)  {alert("Your answer:"+x);txt=x;}
                }
            </script>
            Your answer:<br>
            
            <textarea name="ans" onblur=f() cols="40" rows="5"></textarea><br>
            <input type="submit" value="Add answer" onmouseover="f()">
        </form>
<!-- footer -->
